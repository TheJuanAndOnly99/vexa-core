...
The agent API is Vexum's **control plane**. It turns triggers into [dispatches](/core/agents) and streams their output; it does no execution itself — it validates and hands off to the [runtime](/core/runtime). Endpoints are fronted by the gateway, which carries authentication and per-route scopes; the service itself is internal.
...
Workspaces are **flat and equal-rank**; the **active set** is the source of truth for what is mounted and visible, and reads are **addressed by `slug`** (a no-slug read is a deprecated legacy form that resolves to the seed-slot storage dir — don't rely on it).

`GET /api/workspace/active` → `{ subject, active: [{ slug, repo, ref, role, path, write, primary, name }] }` (the ordered mount set — `primary` marks the agent's cwd, the first active) · `GET /api/workspace/tree?slug=&hidden=` → `{ "files": ["kg/entities/..."] }` · `GET /api/workspace/file?path=&slug=` → `{ "path": "...", "content": "..." }` (`404` if absent) · `GET /api/workspace/git?slug=` → `{ branch, changes, commits }` · `POST /api/workspace/upload` (multipart `files`, ≤25 MB each) → `{ "files": [{ name, path }] }` · `POST /api/workspace/init` → `201` `{ workspace, seeded, already_initialized }` (idempotent) · `GET /api/workspace/attached` → `{ active, slots }` · `POST /api/workspace/new` → `201` create a fresh seeded workspace and add it to the active set · `POST /api/workspace/activate` `{ repo?, ref?, slug?, token? }` / `POST /api/workspace/{slug}/deactivate` → switch a workspace on/off (additive — nothing else is parked) · `POST /api/workspace/swap` `{ repo?, ref?, token? }` → legacy attach-and-park (omit `repo` to swap back to the seed).

(All workspace routes derive `subject` from `X-User-Id` — no `subject` query parameter. Shared-workspace membership, invites, and per-member roles are managed via `/api/workspace/shared/*` and the terminal's manage panel.)