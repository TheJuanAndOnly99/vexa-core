...
**Chat** is a [dispatch](/core/agents) fired _now_: you send a message, an agent works your **active workspaces** — every meeting, email, and note already compiled there — and answers, streaming back live. This is not retrieval over a document store; each workspace is a maintained Markdown knowledge base the agent reads and edits like a developer in a repo.

In the terminal, replies are **connected to the knowledge**: entity mentions render as `[[Title]]` chips and workspace file references as links — click any of them to open the doc, resolved across all active workspaces. Conversation history is private: it lives in your `_system` tier, never inside a (possibly shared) workspace.
...
<Note>
  A `rejected` frame means governance blocked the write. Trusted chat may write; untrusted triggers (email, web) are propose-only — see [Governance](/architecture/governance) and [Troubleshooting](/troubleshooting#agent-write-was-rejected).
</Note>